...
tmp_new="$(mktemp)"
{
  ...
  echo "${INGRESS_VIP} ${DOMAIN_HUBBLE}"
  echo "${MARKER_END}"
} > "${tmp_new}"
...
log_info "  ${INGRESS_VIP} ${DOMAIN_HUBBLE}"
...
log_info "Pipeline complete. Browse to https://${DOMAIN_NEXTCLOUD}, https://${DOMAIN_HOMEASSISTANT}, https://${DOMAIN_ONLYOFFICE}, https://${DOMAIN_ARGOCD}, https://${DOMAIN_HUBBLE}"
